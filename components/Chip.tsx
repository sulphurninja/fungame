import React from "react";
var classNames = require("classnames");

let Chip = function (props: {
  currentItemChips: any;
  currentItem: any;
  leftMin: number | undefined;
  leftMax: number | undefined;
  topMin: number | undefined;
  topMax: number | undefined;
}): JSX.Element {
  var currentItemChips = props.currentItemChips;
  var currentItem = props.currentItem;

  function randomNumber(min: number, max: number) {
    return Math.random() * (max - min) + min;
  }

  function getChipClasses(chip: number) {
    var cellClass = classNames({
      "chip-5000-placed": chip === 5000,
      "chip-1000-placed": chip === 1000,
      "chip-500-placed": chip === 500,
      "chip-100-placed": chip === 100,
      "chip-50-placed": chip === 50,
      "chip-10-placed": chip === 10,
      "chip-5-placed": chip === 5,
      "chip-1-placed": chip === 1,
      chipValueImage: true,
    });

    return cellClass;
  }

  if (currentItemChips !== undefined) {
    var total = 0;
    var chipData = currentItemChips;
    const chipsImgs = [];
    var currentChipPlaced = 0;
    const chipValues = [5000, 1000, 500, 100, 50, 10, 5, 1];

    for (const currentChip of chipValues) {
      while (total < chipData.sum) {
        if (chipData.sum - total >= currentChip) {
          var calc = currentChip;
          total += calc;
          currentChipPlaced = 1;
        } else {
          break;
        }

        var leftMin = -10;
        if (props.leftMin !== undefined) {
          leftMin = props.leftMin;
        }
        var leftMax = 10;
        if (props.leftMax !== undefined) {
          leftMax = props.leftMax;
        }
        var topMin = -30;
        if (props.topMin !== undefined) {
          topMin = props.topMin;
        }
        var topMax = 0;
        if (props.topMax !== undefined) {
          topMax = props.topMax;
        }

        for (let i = 0; i < currentChipPlaced; i++) {
          var key =
            currentItem.type +
            "_" +
            currentItem.value +
            "_" +
            currentChip +
            "_" +
            i;
          let style = {
            top: "0px",
            left: "0px",
          };
          style.left = randomNumber(leftMin, leftMax) + "px";
          style.top = randomNumber(topMin, topMax) + "px";
          chipsImgs.push(
            <div
              key={key}
              style={style}
              className={getChipClasses(currentChip)}
            ></div>
          );
        }
      }
    }

    return <div className={"chipValue"}>{chipsImgs}</div>;
  } else {
    return <></>;
  }
};

export default Chip;
